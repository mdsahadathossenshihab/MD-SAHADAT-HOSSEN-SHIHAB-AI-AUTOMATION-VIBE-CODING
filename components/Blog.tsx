...
import { ArrowRight, Calendar, Image as ImageIcon, Sparkles, AlertCircle, ArrowLeft } from 'lucide-react';
import { BlogPost, BlogProps } from '../types';
import { supabase, DEMO_POSTS } from '../services/supabaseClient';

export const Blog: React.FC<BlogProps> = ({ isAdmin, onPostClick, isFullView = false, onViewAll, onBack }) => {
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        {isFullView && (
          <nav className="mb-8 flex items-center justify-between">
            ...
            <h1 className="text-xl font-black text-slate-900 uppercase tracking-widest">Blog Archive</h1>
          </nav>
        )}
        ...
        <div className={`bg-white rounded-[40px] p-8 md:p-16 shadow-xl shadow-slate-200/50 border border-slate-100 relative overflow-hidden ${isFullView ? 'min-h-[80vh]' : ''}`}>
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                 ...
                 <span className="text-indigo-600 font-bold tracking-widest text-xs uppercase font-mono">
                   {isFullView ? 'All Articles' : 'Knowledge Hub'}
                 </span>
              </div>
              <h2 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight">
                {isFullView ? 'Complete Archive' : 'Latest Insights'}
              </h2>
            </div>
          ...
          {!isLoading && usingDemoData && (
             <div className="mb-8 p-4 bg-blue-50 border border-blue-100 rounded-2xl flex items-center gap-3 text-blue-700 text-sm animate-fade-in-up">
                <AlertCircle size={20} />
                <span><strong>Demo Mode:</strong> Showing sample posts because the database connection is unavailable.</span>
             </div>
          )}
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {isLoading ? (
              ...
              <div className="col-span-3 text-center py-24 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                <p className="text-slate-400 font-bold mb-2">No Content Yet</p>
                <p className="text-slate-500 text-sm">Automated posts will appear here via n8n.</p>
              </div>
            )}
          </div>
